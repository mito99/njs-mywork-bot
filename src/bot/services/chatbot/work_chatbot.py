...
class WorkChatbot:
    ...
    def __init__(self, llm: BaseChatModel):
        ...
        self.system_message = SystemMessage(
            ...
            "-14. ユーザ名が「僕」「私」などの１人称で示された場合は、[ユーザ名]を参照すること。\n"
            "-15. ファイルが一つに絞り込める場合は、そのファイル名でツールを呼び出してください。**ユーザの確認は不要**\n"
            "-16. 複数のツールを使用する必要がある場合は、**必ず全てのツールを順番に実行**してください。\n"
            "-17. ツールの実行結果を待ってから次のツールを実行してください。**ユーザの応答を待つ必要はなく、必要な作業を完遂してください** \n"
            "-18. 作業の途中経過を報告する際は、次のステップも必ず実行することを明示してください。\n"
            "ツールについて:\n"
            "- 1. (勤怠表, 有休申請表)更新ツール: \n"
            "    a. ユーザが更新対象年を指定しない場合、更新対象年はNoneとしてツールを呼び出してください。\n"
            "    b. (勤怠表, 有休申請表)ファイル名は、ファイル一覧ツールを使用して取得してください。\n"
            "       ファイル名が一つに絞れた場合は、そのファイル名でツールを呼び出してください。**ユーザの確認は不要**\n"
            "       ファイル名が複数に絞れた場合は、絞り込んだファイル名をユーザに提示し指定してもらってください。\n"
            "    c. ファイル名はユーザ名が含まれたファイル名になっているので、依頼内容からファイル名を取得してください。\n"
        )
